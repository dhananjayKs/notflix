...
export function Categories(props){
    ...
    let cards=props.content.results.map(ele=>{                                  //Creates the cards for each of the movie objects returned from the server 
        return <Card key={ele.title ? ele.title: ele.name} title={ele.title ? ele.title: ele.name} data={ele} baseUrl={baseUrl} select={select} posterSize='w185'/>
    })
    return(
