...
let config={                                                                                //Default Configuration data 
    images: {
...
let defMovies={                                                                             //Default movies data
    "top_rated": {
        "page": 1,
        "results": [{
            "adult": false,
...
let showDetails={                                                                           ////Default show details data
    backdrop_path: '/7UKgB00W2zFeNQl42SJ3XkJXtql.jpg',
    created_by: [
      {
        id: 2099609,
    ...
    last_episode_to_air: {
      ...
      id: 3099443,
      name: '',
      overview: "As a new day dawns, Moordale's fate hangs in the balance. Aimee spills. Eric confesses. Otis haunts the hospital. Honesty matters now, more than ever.",
      production_code: '',
      season_number: 3,
    ...
    seasons: [
      ...
      },
      {
        ...
        id: 136055,
        name: 'Season 2',
        overview: 'Otis finally loosens up—often and epically—but the pressure’s on to perform as chlamydia hits the school and mates struggle with new issues.',
      ...
      },
      {
        ...
        id: 158823,
        name: 'Season 3',
        overview: 'Word of the "sex school" gets out as a new head teacher tries to control a rambunctious student body and Otis attempts to hide his secret hookup.',
        poster_path: '/8j12tohv1NBZNmpU93f47sAKBbw.jpg',
        season_number: 3
      }
    ],
...
let movieDetails={                                                                          //Default movie details data
    "adult": false,
    ...
    "overview": "Markus, a deployed military man, has to go home to his teenage daughter, Mathilde, when his wife dies in a tragic train accident. It seems to be plain bad luck - but it turns out that it might have been a carefully orchestrated assassination, which his wife ended up being a random casualty of.",
    "popularity": 24.432,
...
let selected={                                                                              ///Default selected item data
    "adult": false,
    "backdrop_path": "/mHU833IrBRfZ3hhzEQsiiw2D8Nc.jpg",
    "genre_ids": [
        27,
...
let search={                                                                                //Default search data
    page: 1,
    results:[
        {
            adult: false,
...
export function reducer(state=data,action){
    if(action.type==='SELECT'){                                                     //set selected item data reducer 
        return {...state,
            selected: action.payload.selected}
    }
    if(action.type==='FETCH_MOVIES'){                                               ////set fetched movie data reducer 
        return {...state,
            movies: action.payload}
    }
    if(action.type==='FETCH_TV'){                                                   //set fetched show data reducer 
        return {...state,
            tv: action.payload}
    }
    ...
    if(action.type==='FETCH_CONFIG'){                                               //set fetched Configuration data reducer 
        return {...state,
            config: action.payload}
    }
    if(action.type==='TV_SHOW_DETAILS'){                                            //set fetched tv Show data reducer 
        return{
            ...state,
            showDetails:action.payload
        }
    }
    if(action.type==='MOVIE_DETAILS'){                                              //set fetched movie data reducer
        return{
            ...state,
            movieDetails:action.payload
        }
    }
    if(action.type==='SEARCH'){                                                     ////set fetched search data reducer
        return{
            ...state,
            search:action.payload
        }
    }
    if(action.type==='SELECTED_TYPE'){                                              //set selected item data type reducer
        return {...state,
